...
"use strict";
    ...
    if(!FE.$els.pageOverlay.length) {
        FE.$els.pageOverlay = $('<div class="page-overlay" id="pageOverlay" />').appendTo(FE.$els.mainSite);
    }

    if(!FE.$els.backToTop.length) {
        FE.$els.backToTop = $('<a href="#top" title="Back To Top" class="main-back-to-top" id="mainBackToTop" />').appendTo('#mainContent');
        ...
        $(window).on('scroll.toggleBackToTop', homebrew.utils.throttle(function() {
            FE.$els.backToTop.toggleClass('is-shown', $(window).scrollTop() > $(window).height()*0.5);
        }, 50));
    }
    ...
    $.extend(true, FE, {
        ...
        showAlertPopup : function(args, method) {
            ...
            if($('body').hasClass('popup-is-shown')
            ...
            var $title = $popup.find('.title'),
                ...
                $prevPopup = $('.popup-is-shown').find('.popup.is-shown');
            ...
            $popup.find('.okay-button, .cancel-button')
                ...
                .on('click.alert', function() {
                    ...
                    if(typeof args.onConfirm === 'function') {
                        if(method === 'confirm') {
                            args.onConfirm($(this).is('.okay-button'));
                        } else {
                            args.onConfirm();
                        }
                    }
        ...
        HeightSlider : function(heightSlider) {
            if(typeof heightSlider === 'string') heightSlider = $(heightSlider);
        ...
        initContentTabbers : function() {
            ...
            $('.js-togglerify-tabs').each(function() {
                var $this = $(this),
        ...
        initDropdownSwitchers : function($scope) {
            var $holders = ($scope && $scope.length)
                               ...
                               : $('.dropdown-switcher-holder');

            if(!$holders.length) return;

            $holders.each(function() {
                var $dropdown = $(this).find('.dropdown-switcher'),
                    $contents = $(this).find('.dropdown-content');
        ...
        initNumberTabbers : function() {
            $('.number-tabber').each(function() {
                var $tabs = $(this).find('.number-tabber__tabs').children('li'),
                    $contents = $(this).find('.number-tabber__contents').children('li');
        ...
        initPage : function() {
            $('.js-dropdownify').dropdownify();
            $('.js-inputify').inputify();

            $.each(FE, function(key, value) {
                if(key.indexOf('init') === 0 && key !== 'initPage') value();
            });

            FE.loadXMLForm($('.js-xml-form-holder'));
            FE.loadXMLTable($('.js-xml-tbl-holder'));
            FE.loadXMLLocations($('.js-xml-locations-holder'));

            if(FE.browser.ie) $('[placeholder]').placeholderify();
        },
        /* End initPage */
        ...
        initSelfTogglers : function() {
            ...
            $('.self-toggler__toggler').togglerify({
                singleActive: false,
                slide: true,
                content: function() {
                    return $(this).next();
        ...
        initSectionContentToggler : function() {
            ...
            var $sectionContentTogglers = $('.section-content-toggler');
            if(!$sectionContentTogglers.length) return;

            var $sectionToggleableContents = $('.section-toggleable-content'),
            ...
            $sectionContentTogglers
                ...
                .each(function(index) {
                    var $dropdownOption = $(['<option>', $.trim($(this).find('.toggler__title').text()), '</option>'].join(''));
                    $dropdownOption.data('section-content-toggler', $(this));
                    $dropdownOption.appendTo($sectionDropdown);
                });

            $('<div class="column show-for-small-down" />')
            ...
            $sectionDropdown
                ...
                .on('change', function() {
                    var $thisToggler = $(this).find('option:checked').data('section-content-toggler');
        ...
        initSmallNav : function() {
            if(Modernizr.touch || $('#mainSmallNav').data('initSmallNav') === true) return;

            $('#mainSmallNav').data('initSmallNav', true);

            var prevScrollTop = $(window).scrollTop(),
                navHeight = $('#mainSmallNav').height() + $('#mainNav').height();
            ...
            $(window)
                ...
                .on('scroll.navHider', function() {
                    ...
                    $('body').toggleClass('main-header-is-partially-hidden', partialHiddenBool);
                    $('body').toggleClass('main-header-is-hidden', hiddenBool);
        ...
        initTooltips : function() {
            var $els = $('.icon--question[title], .js-tooltipify');
        ...
        initQuickLinks : function() {
            var $quicklinksToggler = $('#mainNav__quicklinksToggler'),
                $quicklinks = $('#mainNav__quicklinks');
                ...
                .on({
                    click : function(e) {
                        ...
                        if($thisToggler.hasClass('is-toggled')) {
                            ...
                            var heightLimit = $(window).height() - $('#mainNav').height(),
                                contentHeight;
                    ...
                    toggleOn : function(e, $thisToggler, $target) {
                        $(document).on(mouseupEv, function(e) {
                            var container = $thisToggler.add($target);
                        
                            if(!container.is(e.target) && container.has(e.target).length === 0) {
                                $(document).off(mouseupEv);
                                $target.data('togglerify').toggleOff();
                            }
                        });

                        $(window).on(resizeEv, function() {
        ...
        loadXMLForm : function($holders) {
            ...
            $holders.each(function() {
                var $thisHolder = $(this);
                if($.data(this, 'loaded-xml-form') === true) return;
            ...
            function makeXMLForm(data, $holder) {
                ...
                var $hiddenIframe = $('<iframe data-submitted="false" name="hiddenIframe_' + uniqueID + '" style="display: none;" />').appendTo($xmlForm);
                ...
                $xmlForm.validify().on('submit', function(e) {
                    var validifyData = $.data(this, 'validify');
                    ...
                    if(validifyData.valid !== true) {
                        ...
                        var $errorInput = $(this).find('.is-failed').eq(0);
            ...
            function getText($node) {
                return ($node.length) ? $.trim($node.text()) : '';
            }
        ...
        loadXMLTable : function($holders) {
            ...
            function makeXMLTable(data, xmlStyles, $holder) {
                    ...
                    if(thisWorksheetName.indexOf('dropdown') === 0) {
                        $thisWorksheet.find('Data').each(function() {
                            var $Data = $(this),
                                ...
                                DataText = $(this).text().toLowerCase();

                            switch(DataText) {
                                case 'dropdown label':
                                    dropdown.__label = $(this).closest('Row').next('Row').find('Data').text();
                                break;

                                case 'option label':
                                    $(this).closest('Row').nextUntil().each(function() {
                                        var optionLabelText = $(this).find('Data').eq(DataIndex).text();
                                        if(optionLabelText === '') return;
                                        dropdown[$(this).find('Data').eq(DataIndex).text()] = $(this).find('Data').eq(DataIndex+1).text().toLowerCase();
                                    });
                                break;
                            }
                        });
                ...
                if($.isEmptyObject(dropdown)) {
                    ...
                    $.each(worksheets, function(key, value) {
                    ...
                    $.each(dropdown, function(key, value) {
                        ...
                        for(var i = valueSplitArray.length-1; i > -1; i--) {
                            valueSplitArray[i] = $.trim(valueSplitArray[i]);
                        }
            ...
            function makeTableString($worksheet, xmlStyles) {
                ...
                function makeCellString($cell, cellIndex, log) {
                    ...
                    var cellStyle = $.extend({}, defaultStyle),
                    ...
                    if(xmlStyles[$thisCell.attr('ss:StyleID')]) {
                        $.extend(cellStyle, xmlStyles[$thisCell.attr('ss:StyleID')]);
                    }
            ...
            function makeCellInlineFormats($ssData) {
                 ...
                 */
                var ssDataChildNodes = $.makeArray($ssData[0].childNodes),
                    contentsStr = [];
        ...
        loadXMLLocations : function($holders) {
            ...
            function makeLocationsList(data, $holder) {
                var $locations = $(data).find('obj'),
                    regions = [];

                $locations.each(function() {
                    var $region = $(this).find('region'),
                        regionStr = $.trim($region.text());
                                ...
                                '<div class="column">',
                                    '<div class="location">',
                                        '<div class="location__name">',
                                            $.trim(regions[i].$els[0].find('storeName').text()),
                                        '</div>',

                                        '<div class="location__address">',
                                            $.trim(regions[i].$els[0].find('storeAddress').text()),
                                        '</div>',

                                        '<div class="location__footer">',
                                            '<a href="https://maps.google.com/maps?daddr=',
                                                $.trim(regions[i].$els[0].find('storeLat').text()), ',',
                                                $.trim(regions[i].$els[0].find('storeLng').text()), '" target="_blank">',
                                                'Get Directions',
                                            '</a>',
                                        '</div>',
                                    '</div>',
                ...
                if(regions.length > 1) {
                    ...
                    var $switcher = $(switcherStrArr.join(''));
                    ...
                    $holder.find('select').on('change', function() {
                        var targetIndex = parseInt($(this).val(), 10);
        ...
        scrollTo : function($obj, callback) {
            ...
            if(targetScrollPos < $(window).scrollTop()) {
                targetScrollPos -= $('#mainNav').outerHeight();

                if($('#mainSmallNav').css('display') !== 'none'
                && !$('#mainHeader').hasClass('is-partially-hidden')) {
                    targetScrollPos -= $('#mainSmallNav').outerHeight();
                }
            }
    ...
    $.extend(FE.FootnoteAnchor.prototype, {
                    ...
                    $(document)
                        ...
                        .on('mouseup.notesFauxBlur', function(e) {
                            if(!$thisFootnote.is(e.target) && $thisFootnote.has(e.target).length === 0) {
                                $(document).off('mouseup.notesFauxBlur');
    ...
    $.extend(FE.Hasher.prototype, {
        get : function(args) {
            ...
            while(splitHash.length) {
                ...
                dataObj[$.trim(keyValuePairs[0])] = decodeURIComponent($.trim(keyValuePairs[1]));
            }
        ...
        setData : function(data) {
            if(!data || !$.isPlainObject(data)) return this;
        ...
        set : function(args) {
            ...
            || typeof sequence === 'function') {
                $.each(instance.data, function(key, value) {
                    if(key === '' || value === '') return;
                    hashKeys.push(key);
                });
    ...
    $.extend(FE.StickyColumn.prototype, {
        callbacks : $({}),

        hasMousewheel : Boolean($.fn.mousewheel),
        ...
        load : function(args) {
            ...
            if(!proto.hasMousewheel) {
                if(!proto.loadingMousewheel) {
                    ...
                    $.getScript(FE.baseURL + 'js/plugins/jquery.mousewheel.min.js', function() {
        ...
        determineState : function() {
            ...
            instance.offsetTop = ($('#mainHeader').css('position') === 'fixed') ? /*instance.$els.mainNav.height()*/ 0 : 0;
        ...
        updatePosition : function() {
            ...
            if($el.hasClass(instance.classes.stickBottom) || !$el.hasClass(instance.classes.sticky)) {
                ...
                $el.css('top', $(window).scrollTop() - instance.$comparee.offset().top + instance.offsetTop + 'px');
            }
        ...
        watch : function() {
            ...
            instance.$el.on('mousewheel', function(e) {
                ...
                if(e.deltaY < 0) {
                    var bottomDiff = $(window).scrollTop() + $(window).height() - $(this).offset().top - $(this).height();
                    ...
                    var topDiff = $(window).scrollTop() + instance.offsetTop - $(this).offset().top;
